...
Player::Player(CameraObject* camera, World* w) {
    ...
    atlasTexture = LoadTexture(ResolveGamePath({ "Projects", "TheGame", "Assets", "Glitcht", "glitcht_animation.dds" }).string().c_str());
    SetTextureFilter(atlasTexture, TEXTURE_FILTER_BILINEAR);

    // Carrega o arquivo JSON
    std::ifstream f(ResolveGamePath({ "Projects", "TheGame", "Assets", "Glitcht", "glitcht_animation.json" }).string());
    if (!f.is_open()) {
